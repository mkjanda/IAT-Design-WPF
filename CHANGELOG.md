...
- Combined-key conjunction ("or") is 55% of the smaller adjacent term (min 10pt),
  not the same em as the labels. `TextExportProcessor` writes one PNG per key via
  `RenderKeyToBitmap` → `Key.ResolveDisplayLines`. A 24pt pair ships a ~13pt "or"
  so FitContain cannot restore the oversized conjunction. Blocks/Instructions
  preview binds the same computed size.
...
- Response key and text-stimulus PNGs no longer crop. Rasterize to ink bounds
  ...
  (that painted "Flower" at negative X and shipped "wer").
...
- GManifest wire shape: `Manifest` is flattened so XmlSerializer emits `ProductKey`, `IATName`, then `File*`. `ManifestType` / `ResourceType` use `[XmlEnum]` tokens that match the XSD (`ErrorMark`, `Javascript`, `FileManifest`, `ItemSlideManifest`)
- `IFileManifestBuilder.AddFile` takes an explicit `resourceId`. ErrorMark is 1000; key outlines are separate files at 1001 / 1002
- `BeginIATBlock.NumPresentations` is written from `Block.NumPresentations`
- `TextExportProcessor` encodes the PNG before the stream is disposed
- `Manifest` is registered in `XmlDeserializationService`
- Image resize is contain-fit, not fill/crop. `GetResizedBitmap` letterboxes onto the target canvas; `RenderSlide` draws the stimulus into a centered fitted rect. The editor already used `Stretch="Uniform"` — slides and export now match it.
- Response-key labels shrink to fit their layout rect. `RenderTextToBitmap` reduces em-size until the stacked text fits; Blocks and Instructions previews wrap the key `TextBlock` in a `Viewbox` so `"Insects"` is no longer clipped to `"Inse"`
- Block instructions, Text / Keyed / Mock Item screen bodies, continue prompts, and mock text stimuli use the same contain-fit: preview `Viewbox StretchDirection="DownOnly"` (authored size when it fits, scale down when it does not). Slides already go through `RenderTextToBitmap` shrink-to-fit.
